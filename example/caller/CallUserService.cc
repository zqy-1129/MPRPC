...
int main(int argc, char **argv)
{
    ...
    stub.Login(&controller, &request, &response, nullptr);   // RpcChannel调用其的callMethod方法集中来做所有Rpc方法调用的参数序列化和网络发送
    ...
    if (controller.Failed())
    {
        std::cout << "rpc register failed! error: " << controller.ErrorText() << std::endl;
        return 0;
    }
    else
    {
